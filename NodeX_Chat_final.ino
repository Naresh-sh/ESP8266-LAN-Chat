...
// ── CHAT LOGIN PASSWORD (change karo yahan) ──
...
// ════════════════════════════════════════════════════════
//  NEW: HTTP login verify endpoint
...
// ════════════════════════════════════════════════════════
//  HTML PAGE  (password check is now via /auth endpoint)
// ════════════════════════════════════════════════════════
...
#loginErr{
  ...
  background:#2a0000;border:1px solid #ff000055;
...
<div id="login-screen">
  <div class="login-box">
    ...
    <div id="loginErr"></div>
    <button class="login-btn" id="loginBtn" onclick="login()">AUTHENTICATE</button>
    <p style="color:#333;font-size:10px;margin-top:12px">NodeX_Tech &copy; LAN Only</p>
  </div>
</div>

<div id="main-ui">
  <div id="topBar">
    <div id="topBarLeft">NODEX LAN CHAT</div>
    <div id="topBarRight">
  ...
  <div class="main-body">
    <div class="sidebar">
      <div class="sidebar-header">NODEX CHATS</div>
      <div id="userList"></div>
    </div>
...
function soundSent(){try{var a=getACtx(),o=a.createOscillator(),g=a.createGain();o.connect(g);g.connect(a.destination);o.type="sine";o.frequency.setValueAtTime(600,a.currentTime);o.frequency.exponentialRampToValueAtTime(1100,a.currentTime+0.12);g.gain.setValueAtTime(0.25,a.currentTime);g.gain.exponentialRampToValueAtTime(0.001,a.currentTime+0.18);o.start(a.currentTime);o.stop(a.currentTime+0.18);}catch(e){}}
function soundReceived(){try{var a=getACtx();[0,0.1].forEach(function(d,i){var o=a.createOscillator(),g=a.createGain();o.connect(g);g.connect(a.destination);o.type="triangle";o.frequency.setValueAtTime(i===0?880:1100,a.currentTime+d);g.gain.setValueAtTime(0.2,a.currentTime+d);g.gain.exponentialRampToValueAtTime(0.001,a.currentTime+d+0.15);o.start(a.currentTime+d);o.stop(a.currentTime+d+0.15);});}catch(e){}}
function soundJoin(){try{var a=getACtx(),o=a.createOscillator(),g=a.createGain();o.connect(g);g.connect(a.destination);o.type="sine";o.frequency.setValueAtTime(300,a.currentTime);o.frequency.exponentialRampToValueAtTime(900,a.currentTime+0.25);g.gain.setValueAtTime(0.15,a.currentTime);g.gain.exponentialRampToValueAtTime(0.001,a.currentTime+0.3);o.start(a.currentTime);o.stop(a.currentTime+0.3);}catch(e){}}
function soundDelete(){try{var a=getACtx(),o=a.createOscillator(),g=a.createGain();o.connect(g);g.connect(a.destination);o.type="sawtooth";o.frequency.setValueAtTime(400,a.currentTime);o.frequency.exponentialRampToValueAtTime(80,a.currentTime+0.2);g.gain.setValueAtTime(0.2,a.currentTime);g.gain.exponentialRampToValueAtTime(0.001,a.currentTime+0.22);o.start(a.currentTime);o.stop(a.currentTime+0.22);}catch(e){}}
...
"😈":"devil evil smiling","💩":"poop","❤":"heart love red","🧡":"orange heart",
...
"🚀":"rocket space launch","🌍":"earth world globe","🔮":"crystal ball magic",
"💻":"laptop computer","📱":"phone mobile","🔑":"key","🔒":"locked secure",
"🛡":"shield protect","⚔":"swords fight","💣":"bomb","🍕":"pizza food",
"🍔":"burger food","🍟":"fries food","🌮":"taco food","🍜":"ramen noodles",
"🎂":"cake birthday","☕":"coffee drink","🍺":"beer drink","⚠":"warning danger",
"✅":"check done","❌":"cross wrong no","💤":"zzz sleep","🥷":"ninja secret",
"🐉":"dragon","🦊":"fox","🐺":"wolf"
};
var TABS=[
...
{icon:"🎮",label:"Fun",keys:["game","rocket","earth","magic","guitar","music","pizza","burger","cake","coffee","beer","ninja","dragon","wolf","fox"]},
{icon:"💻",label:"Tech",keys:["laptop","phone","key","lock","shield","sword","bomb","robot","brain"]}
];
var emojiAllList=[];
...
nameSpan.textContent=n.ssid+(n.enc?" [lock]":"");
...
if(u===""){
errDiv.textContent="Name khali nahi rakh sakte!";
errDiv.style.display="block";
setTimeout(function(){errDiv.style.display="none";},3000);
return;
}
if(p===""){
errDiv.textContent="Password daalo!";
errDiv.style.display="block";
setTimeout(function(){errDiv.style.display="none";},3000);
return;
}
...
errDiv.textContent="Galat password! Dobara try karo.";
errDiv.style.display="block";
btn.disabled=false;
...
errDiv.textContent="Server se connect nahi ho pa raha. Refresh karo.";
errDiv.style.display="block";
btn.disabled=false;
btn.textContent="AUTHENTICATE";
setTimeout(function(){errDiv.style.display="none";},3500);
...
socket.onerror=function(){showToast("WebSocket error!");};
}
...
div.style.cssText="position:absolute;top:-50px;left:0;background:#111;border:1px solid #444;border-radius:20px;padding:5px 8px;display:flex;gap:6px;z-index:100;box-shadow:0 3px 12px rgba(0,0,0,0.5);";
...
t.style.cssText="position:fixed;top:18px;left:50%;transform:translateX(-50%);background:#1a1a1a;border:1px solid #ff0000;color:#ff4444;padding:9px 18px;border-radius:10px;z-index:9999;font-size:13px;font-weight:bold;box-shadow:0 0 15px rgba(255,0,0,0.3);";
...
void setup(){
  ...
  server.on("/auth",       HTTP_POST, handleAuth);      // NEW
